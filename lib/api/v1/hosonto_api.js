...
module.exports = function (app, myConn) {
  ...
  app.get(
    ...
    async function (req, res) {
      ...
      try {
        ...
        params = await loadAutoTablePromise(
          params /*,
          function callback(err, sessionId) {
            params["__SessionId"] = sessionId;
            global._params = params;
            log.debug("LoadTable Params=" + util.inspect(params));
  ...
  app.post("/autoLoadData", isAutoDataAllowed, function (req, res) {
    try {
      ...
      if (req.session && req.session.passport) {
        params["session_user"] = req.session.passport.user;
      }
      ...
      myConn.loadAutoTable(params, function callback(err, sessionId) {
        params["__SessionId"] = sessionId;
        global._params = params;
  ...
  let callFunction = async function (req, res, params, fn) {
    try {
      let passed = await fn(params, req, res);
      ...
      if (params["__UseServerSideFunction"] === true) {
        ...
        log.trace("\n Before save params: " + util.inspect(passed));
        myConn.saveAutoParams(passed, (err, params) => {
          log.trace("\n executeEvent::  Loading params" + util.inspect(params));
  ...
  app.post("/executeEvent", function (req, res) {
    ...
    try {
      ...
      if (
        ...
        try {
          callFunction(req, res, params, fn);
        } catch (error) {
        ...
        myConn.saveAutoParams(newParams, (err, params) => {
          myConn.loadAutoParams(params, (err, params) => {
            if (!err) {
      ...
      console.log("\n\n\n Err occurred" + JSON.stringify(error));
      sendHttpResponse(res, 500, error);
    }
  });
