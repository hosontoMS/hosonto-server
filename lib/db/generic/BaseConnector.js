...
const updateParam = async function (
  ...
  if (table) {
    try {
      qry = await context.dbHelper.decodeFields(qryParam, user_id, [
        "_id",
        "_owner_id",
        "_shared_ids",
      ]);
...
BaseConnector.prototype = {
  ...
  saveAutoParams: async function (context, params, callbackSaveAutoParams) {
    ...
    try {
      ...
      log.debug("\nError in SaveAutoParams, err: " + util.inspect(err));
  ...
  saveParamsForValidSessionId: function (__SessionId, params, callback) {
    ...
    const user_id = params["session_user"] && params["session_user"]._id;
      ...
      if (arr.length > 1 && !arr[1]) {
        ...
        if (typeof params[p] === "object" && params[p][0]) {
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            updateParam(me, qry, upd, arr[0], p, idx, user_id);
          }
          ...
          if (!isNumericIndex) {
            ...
            updateParam(me, qry, upd, arr[0], p, null, user_id);
          }
