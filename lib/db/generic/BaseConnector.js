...
BaseConnector.prototype = {
  ...
  loadAutoTable: async function (params, cback) {
    ...
    for (currentParam in params) {
      ...
      if (splitTABLE.length > 1 && !splitTABLE[1]) {
        ...
        // let currentTable = index.slice(0, index.indexOf("_TABLE"));
        let [
          filteredFields,
          idFields,
          sharedFields,
  ...
  loadAutoParams: function (params, callback) {
    if (!params["__SessionId"]) {
        ...
        ((that, callB, params) => {
          return function (err, nextSeq) {
            if (err) {
              ...
              log.debug(
                "\n\n\nloadAutoParams:: Next SessionID" + nextSeq["seq"]
              );
  ...
  loadParamsForValidSessionId: function (params, callback) {
    log.debug("\n\n\ninside loadAutoParams:");
    log.trace("params :: " + util.inspect(params));

    if (
      params["__SessionId"] === null ||
      params["__SessionId"] === "" ||
    ...
    this.connector.findDefault(
      ...
      (err, res) => {
        if (err) {
          log.debug("loadAutoParams:error" + err);
        } else {
          log.debug("   loadAutoParams:SUCCESS  " + res);
  ...
  saveAutoParams: function (params, callbackSaveAutoParams) {
    ...
    if (params["readOnly"] === true) {
      callbackSaveAutoParams(params, params.__SessionId);
      return;
    }
  ...
  saveParamsForValidSessionId(__SessionId, params, callback) {
      ...
      promises.push(
        new Promise(async (resolve, reject) => {
          ...
          if (table) {
            ...
            context.connector.update(qryParam, updParam, (err, res) => {
              if (err) {
                reject(err);
              } else {
                resolve(res);
    ...
    Object.keys(params).forEach((p) => {
      ...
      //log.debug(`@@ SAVEPARAMS${p} val ${util.inspect(params[p])} Arr=${util.inspect(arr)} length=${arr.length}`);
